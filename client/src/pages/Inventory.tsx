...
export default function Inventory() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setOpen(false);
      setFormData({ name: "", partNumber: "", quantity: "", threshold: "", unit: "piece", category: "", supplier: "", notes: "" });
      setEditingId(null);
      refetch();
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-600 mt-1">Track spare parts and stock levels</p>
        </div>
      ...
      {lowStock && lowStock.length > 0 && (
        <Card className="border-red-200 bg-red-50">
          <CardHeader>
            ...
            <CardDescription className="text-red-700">{lowStock.length} items are below threshold</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
              {lowStock.map((item: any) => (
                <div key={item.id} className="flex justify-between items-center text-sm">
                  ...
                  <span className="text-red-600">{item.quantity}/{item.threshold}</span>
                </div>
              ))}
            </div>
          </CardContent>
        ...
        <CardContent>
          {isLoading ? (
                  ...
                  {inventory.map((item: any) => {
                    ...
                    return (
                      <tr key={item.id} className={`border-b hover:bg-slate-50 ${isLowStock ? "bg-red-50" : ""}`}>
                        ...
                        <td className="py-2 px-4 font-mono text-xs">{item.partNumber}</td>
                        <td className="py-2 px-4">{item.quantity} {item.unit}</td>
            ...
            <div className="text-center py-8">
              ...
              <p className="text-slate-600">No inventory items found. Add your first item.</p>
            </div>
          )}
        </CardContent>
      </Card>
